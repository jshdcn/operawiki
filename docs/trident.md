...
Trident既闭源，又捆绑了Windows平台的许多独有事物，而微软倾向于在不同的操作系统使用不同的排版引擎，所以在四大排版引擎中，它是唯一未能实现跨平台的一个。其它平台模拟Trident的项目，如IEs4Linux，至今未能取得理想的效果。尽管如此，还是有很多Windows平台应用程序调用Trident来渲染它们的页面，如[傲游](maxthon.md)、MSN Explorer等“壳浏览器”，以及RealPlayer、Windows Media Player这样对排版没有什么要求的客户端软件等。
