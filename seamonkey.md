...
微软在浏览器大战中的胜利使得Mozilla这类产品必须同时面临两个强大对手的竞争。比这更严重的是，由于用户已经习惯于分开使用浏览器和客户端甚至根本不使用客户端，他们在遇到网络套件时往往会茫然不知所措。生不逢时，导致了Mozilla Suite叫好不叫座。在巨大压力面前，Mozilla基金会终于决定分拆Mozilla Suite：

* 浏览器部分形成了Mozilla Firefox。还有一个使用Cocoa环境的Mac OS专版计划，名为Camino；

* 邮件/新闻元件部分形成了Mozilla Thunderbird；
